...
char MyCar_Status[9][17];//小车状态的字串变量
...
int main(){
    ...
    strcpy(MyCar_Status[6],"Yaw:");//MPU6050提供的小车当前YAW值
    strcpy(MyCar_Status[7],"Rol:");//MPU6050提供的小车当前Roll值
    strcpy(MyCar_Status[8],"Pit:");//MPU6050提供的小车当前Pitch值
    ...
    while(1){
        ...
        for (int i = 0; i < 3; i ++){
            MyCar_Status[7][5+i]=((int32_t)temp / main_Pow(10, 3 - i - 1) % 10 + '0');
        }
        ...
        for (int i = 0; i < 3; i ++){
            MyCar_Status[8][5+i]=((int32_t)temp / main_Pow(10, 3 - i - 1) % 10 + '0');
        }
    }
}
